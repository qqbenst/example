...
public class PrimaryKeyPredicateBuilder {
	...
	public static <T> Predicate getPredicate(Root<T> root, CriteriaBuilder cb, Example<T> example,
		...
		ComparisonOperator operator = keySort.isAscending() ? ComparisonOperator.GREATER_THAN_OR_EQUAL
		...
		List<Predicate> predicates = getPredicates("", cb, root, root.getModel(), example.getProbe(),
				...
				new PathNode("root", null, example.getProbe()), operator);
	...
	static List<Predicate> getPredicates(String path, CriteriaBuilder cb, Path<?> from, ManagedType<?> type,
		...
		for (SingularAttribute attribute : type.getSingularAttributes()) {
			...
			if (attribute.isId()) {
				ComparisonPredicate fuck = new ComparisonPredicate((CriteriaBuilderImpl) cb, operator,
						from.get(attribute), attributeValue);
